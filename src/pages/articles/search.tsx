...
export const SearchArticles = () => {
  ...
  const onSubmit: SubmitHandler<{ title: string }> = ({ title }) => {
    if (!title) return;
    const filteredArticles = query.data?.filter((article) =>
  ...
  return (
    <div className="flex w-full grow flex-col items-center justify-start gap-8">
      ...
      {noMatch && (
        <div className="self-start text-left font-serif text-xl text-stone-800">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid w-full grid-cols-3 gap-4">
            {filteredArticles.map((article) => (
              <div
                key={article.id}
                className="flex flex-col gap-2 rounded-2xl border border-neutral-200 bg-neutral-100 p-4"
              >
                <h2 className="text-base capitalize text-stone-800">
                  {article.title}
                </h2>
